...
while ($query->have_posts()) {
    ...
    if(!cvtx_spd_part_of_bbuch($item))
      continue;
?>
<?php if(!$col1): ?>
...
<?php } else if($running_aeantraege && !$col1) {?>
\vspace{1cm}
<?php } ?>
...
\begin{linenumbers}
<?php endif; ?>
\addcontentsline{toc}{subsection}{\protect \hspace{2.3em} \textbf{<?php cvtx_kuerzel($item); ?>} \hfill \textbf{<?php cvtx_antragsteller($item); ?>} \\ <?php cvtx_titel($item); ?> \\ \textit{<?php cvtx_spd_antrag_expl($item); ?>}}
...
<?php cvtx_spd_tex_beschluss1($item); ?>
\par
<?php if(!$col1): ?>
\end{myLN}
...
<?php   }
        ...
        if ($antrag != $this_antrag || !$running_aeantraege) {
            $antrag  = $this_antrag;
?>
            \addcontentsline{toc}{section}{\"Anderungsantr\"age zum <?php cvtx_antrag($item); ?>}
...
\alignL\cell{\begin{myLN}
<?php else: ?>
\begin{linenumbers}
<?php endif; ?>
\addcontentsline{toc}{subsection}{\protect \hspace{2.3em} \textbf{<?php cvtx_kuerzel($item); ?>} \hfill \textbf{<?php cvtx_antragsteller($item); ?>} \\ <?php cvtx_spd_aeantrag_titel($item); ?> \\ \textit{<?php cvtx_spd_result($item); ?>}}
\textbf{<?php cvtx_kuerzel($item); ?>}\\
\textbf{<?php cvtx_antragsteller($item); ?>}\\
\textbf{<?php cvtx_spd_tex_recipient($item); ?>}\\
\newline
\textbf{<?php cvtx_spd_aeantrag_titel($item); ?>}\\
<?php cvtx_spd_tex_beschluss($item); ?>\par
<?php if(!$col1): ?>
\end{myLN}
\setcounter{maxNum}{\value{numberLines}}
\setcounter{cell1}{\value{numberLines}}
...
\vspace{1cm}
<?php endif; ?>
\textbf{<?php cvtx_spd_result($item); ?>}\par
<?php if($col1): ?>
\end{linenumbers}
<?php else: ?>
\end{myLN}
