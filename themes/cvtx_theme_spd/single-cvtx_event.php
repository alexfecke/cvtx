...
<?php get_header(); ?>
    ...
    // Get all TOPs
    global $wpdb;
    $tids = $wpdb->get_col($wpdb->prepare("SELECT DISTINCT p.ID
                                                      FROM $wpdb->posts p
                                                INNER JOIN $wpdb->postmeta p2 ON (p2.post_id = p.ID and p2.meta_key = 'cvtx_top_event')
                                                 LEFT JOIN $wpdb->postmeta cvtxsort ON (cvtxsort.post_id = p.ID and cvtxsort.meta_key = 'cvtx_sort')
                                                     WHERE p2.meta_value IN (%d, '-1')
                                                  ORDER BY cvtxsort.meta_value ASC", $event_id));
    if (!empty($tids)): ?>
        <ul id="antraege">
            ...
            $aids = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT p.ID, pm1.meta_value AS top, p.post_title
                                                            ...
                                                            INNER JOIN $wpdb->postmeta AS pm2 ON (pm2.post_id = p.ID and pm2.meta_key = 'cvtx_antrag_event')
                                                             LEFT JOIN $wpdb->postmeta AS cvtxsort ON (cvtxsort.post_id = p.ID and cvtxsort.meta_key = 'cvtx_sort')
                                                              ...
                                                              ORDER BY cvtxsort.meta_value ASC", array($event_id)));
            ...
            $aeids = $wpdb->get_results("SELECT DISTINCT p.ID, pm1.meta_value AS antrag
                                                    FROM $wpdb->posts AS p
            ...
            foreach($tids as $tid): $top_id = $tid;?>
                <li class="top" id="<?php print get_post_meta($tid,'cvtx_top_short',true);?>">
                    <h3><?php echo get_the_title($tid); ?></h3>
                    <div class="top_info" id="<?php print get_post_meta($tid,'cvtx_top_short',true);?>_info"><?php get_the_content($tid); ?></div>
                    <?php if(isset($new_aids[$top_id]) && !empty($new_aids[$top_id])): ?>
                        <ul>
                        <?php foreach ($new_aids[$top_id] as $aid): $post = get_post($aid); setup_postdata($post); ?>
                            <li class="antrag"><h4><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h4>
                                ...
                                <?php if(!empty($new_aeaids[$antrag_id])): ?>
                                    <ul class="ae_antraege">
                                        ...
                                        <?php foreach($new_aeaids[$antrag_id] as $aeid): ?>
